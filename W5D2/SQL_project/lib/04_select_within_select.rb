...
def richer_than_england
  ...
  SELECT name
  FROM countries
  WHERE (gdp/population) > (2022824000000 / 59600000)
...
#in continents that have belize and belgim 
...
def population_constraint
  ...
  WHERE population > 32000000 
    AND population < 38500000
  SQL
end

def sparse_continents
  # Find every country (name) that belongs to a continent where each country's
